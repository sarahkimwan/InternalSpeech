...
for rep = 1:options.numRep
    ...
    [Xtest, XtrainFull] = dPCA.dpca_getTestTrials(Xtrial, numOfTrials, ...
...
%     CnoiseTrain = diag(sum(bsxfun(@times, SSnoiseSumOverT(:,:), 1./(numOfTrials(:,:)-1)),2));
    ...
    if ~strcmp(options.noiseCovType, 'none')
        CnoiseTrain = dPCA.dpca_getNoiseCovariance(Xtrain, XtrainFull, numOfTrials-1, ...
    ...
    XtestMargs = dPCA.dpca_marginalize(XtestCen, 'combinedParams', options.combinedParams, ...
                    'ifFlat', 'yes');
    ...
    XtrainMargs = dPCA.dpca_marginalize(XtrainCen, 'combinedParams', options.combinedParams, ...
                    'ifFlat', 'yes');
    ...
    for l = 1:length(options.lambdas)
        ...
        [W,V,whichMarg] = dPCA.dpca(Xtrain, options.numComps, ...
