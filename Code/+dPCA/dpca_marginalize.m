...
for subs = 1:length(paramsubsets)
    indRest = paramsubsets{subs}; %current parameter we look at 
    indMarg = setdiff(params, indRest); %parameter we want to "average out"
    YY{subs} = nanmmean(X, indMarg); %average according to the parameters we don't care about
